...
"""
Rule Generator feature cache.

This is a direct port of capa.ida.plugin.cache. The matching/extraction logic
is disassembler-agnostic -- it only relies on the StaticFeatureExtractor
interface -- so the only change from the IDA version is the extractor type hint.
"""
...
class CapaRuleGenFeatureCache:
    def __init__(self, extractor: StaticFeatureExtractor):
        self.extractor = extractor
        self.global_features: FeatureSet = collections.defaultdict(set)

        self.file_node: CapaRuleGenFeatureCacheNode = CapaRuleGenFeatureCacheNode(
            None, None
        )
    ...
    def _find_function_and_below_features(self, fh: FunctionHandle):
        f_node: CapaRuleGenFeatureCacheNode = CapaRuleGenFeatureCacheNode(
            fh, self.file_node
        )

        for bbh in self.extractor.get_basic_blocks(fh):
            bb_node: CapaRuleGenFeatureCacheNode = CapaRuleGenFeatureCacheNode(
                bbh, f_node
            )

            for ih in self.extractor.get_instructions(fh, bbh):
                inode: CapaRuleGenFeatureCacheNode = CapaRuleGenFeatureCacheNode(
                    ih, bb_node
                )
    ...
    def _find_instruction_capabilities(
        ...
        for feature, locs in itertools.chain(
            insn.features.items(), self.global_features.items()
        ):
    ...
    def _find_basic_block_capabilities(
        ...
        for feature, locs in itertools.chain(
            bb.features.items(), self.global_features.items()
        ):
            features[feature].update(locs)
    ...
    def find_code_capabilities(
        ...
        for bb in f_node.children:
            features, bmatches, imatches = self._find_basic_block_capabilities(
                ruleset, bb
            )
        ...
        for feature, locs in itertools.chain(
            f_node.features.items(), self.global_features.items()
        ):
            function_features[feature].update(locs)

        _, function_matches = ruleset.match(
            Scope.FUNCTION, function_features, f_node.address
        )
    ...
    def find_file_capabilities(
        ...
        for func_node in self.file_node.children:
            ...
            func_features, _, _, _ = self.find_code_capabilities(
                ruleset, func_node.inner
            )
            for feature, locs in func_features.items():
                features[feature].update(locs)

        for feature, locs in itertools.chain(
            self.file_node.features.items(), self.global_features.items()
        ):
            features[feature].update(locs)
    ...
    def _get_cached_func_node(
        self, fh: FunctionHandle
    ) -> Optional[CapaRuleGenFeatureCacheNode]:
