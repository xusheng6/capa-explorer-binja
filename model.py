...
"""
Qt model that renders a capa ResultDocument as a tree.

Ported from capa.ida.plugin.model. The rendering logic is identical; the
disassembler-specific bits -- highlighting feature addresses and renaming
functions -- now go through capa.binja.plugin.helpers using the BinaryView the
...
class CapaExplorerDataModel(QtCore.QAbstractItemModel):
    ...
    def reset(self):
        ...
        for idx in range(self.root_node.childCount()):
            ...
            for model_index in self.iterateChildrenIndexFromRootIndex(
                ...
                self.reset_highlighting(model_index.internalPointer(), False)
    ...
    def headerData(self, section, orientation, role):
        """return data for the given role and section in the header"""
    ...
    def setData(self, model_index, value, role):
        ...
        if (
            ...
            for child_index in self.iterateChildrenIndexFromRootIndex(
                ...
                self.reset_highlighting(child_index.internalPointer(), value)
        ...
        if (
            ...
            # user renamed function - update Binary Ninja database and data model
            old_name = model_index.internalPointer().info
            new_name = str(value)

            if helpers.rename_function(
                self.bv, model_index.internalPointer().location, new_name
            ):
    ...
    def render_capa_doc_statement_node(
        ...
        if isinstance(statement, rd.CompoundStatement):
            ...
            # `range` is a weird node, almost a hybrid of statement + feature.
    ...
    def render_capa_doc_by_function(self, doc: rd.ResultDocument):
        """render rule matches by function meaning each rule match is nested under the function where it was found"""
        ...
        for rule in rutils.capability_rules(doc):
            ...
            for ea in match_eas:
                func_ea: Optional[int] = helpers.get_func_start(self.bv, ea)
                if func_ea is None:
                    continue
                ...
                if not matches_by_function.get(func_address, ()):
                    matches_by_function[func_address] = (
                        CapaExplorerFunctionItem(
                            self.root_node, self.bv, func_address, can_check=False
                        ),
                        set(),
                    )
                ...
                CapaExplorerRuleItem(
                    ...
                    len(
                        [
                            ea
                            for ea in match_eas
                            if helpers.get_func_start(self.bv, ea) == func_ea
                        ]
                    ),
                    rule.source,
                    can_check=False,
    ...
    def render_capa_doc_by_program(self, doc: rd.ResultDocument):
        """render rule matches as a flat rule > location hierarchy"""
        for rule in rutils.capability_rules(doc):
            ...
            for location_, match in rule.matches:
                ...
                if capa.rules.Scope.FILE in rule.meta.scopes:
                    ...
                    parent2 = CapaExplorerFunctionItem(parent, self.bv, location)
    ...
    def render_capa_doc_feature(
        ...
        if isinstance(feature, frzf.CharacteristicFeature):
            ...
            if characteristic in ("embedded pe",):
                return CapaExplorerByteViewItem(parent, self.bv, display, location)

            if characteristic in ("loop", "recursive call", "tight loop"):
                return CapaExplorerFeatureItem(parent, display=display)

            return CapaExplorerInstructionViewItem(parent, self.bv, display, location)
